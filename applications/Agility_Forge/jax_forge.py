...
## Version 4: Add cooling effect on the surface nodes, with a simple exponential decay formula based on the time step and a cool_factor parameter. This is applied at each time step in the AutomaticTimeStepperTM, simulating the cooling of the material at the surface during forging.
                ...
                try:
                    ...
                    if vtk_dir is not None and (self.step_count % save_every == 0):
                        ...
                        save_sol(
                            ...
                            self.sol_u,
                            vtk_path,
                            point_infos=[("Displacement", self.sol_u), ("Temperature (°C)", self.sol_dT)],
...
def run_thermo_mech_cylinder_press_two_hits():
    ...
    # Initial temperature profile from the start of hit 1 (linear gradient along x)
    T_top = 1096.1  # °C
    T_bot = 677.4   # °C

    def T_linear_x(point):
        """Linear temperature gradient along axis"""
        x = point[0]
        xc = np.clip(x, 0.0, H)
        return T_bot + (T_top - T_bot) * (xc / H)
